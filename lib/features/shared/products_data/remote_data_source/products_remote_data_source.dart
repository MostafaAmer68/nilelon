...
import 'package:nilelon/features/shared/products_data/models/products_response_model.dart';
...
class ProductsRemoteDataSourceImpl extends ProductsRemoteDataSource {
  ...
  Future<ProductsResponseModel> getFollowedItems(int page, int pageSize) async {
    final data = await apiService.get(
      endPoint:
          '${EndPoint.getFollowedProductsUrl}${HiveStorage.get(HiveKeys.idToken)}${EndPoint.page}$page${EndPoint.pageSize}$pageSize',
    );
    print(data);
  ...
  Future<ProductsResponseModel> getNewInItems(int page, int pageSize) async {
    final data = await apiService.get(
      endPoint:
          '${EndPoint.getNewProductsUrl}${HiveStorage.get(HiveKeys.idToken)}${EndPoint.page}$page${EndPoint.pageSize}$pageSize',
    );
    print(data);
    if (data.statusCode == 200) {
      return ProductsResponseModel.fromJson(data.data as Map<String, dynamic>);
  ...
  Future<ProductsResponseModel> getHandPickedItems(
    ...
    final data = await apiService.get(
      endPoint:
          '${EndPoint.getRandomProductsUrl}${HiveStorage.get(HiveKeys.idToken)}${EndPoint.page}$page${EndPoint.pageSize}$pageSize',
    );
    print(data);
    if (data.statusCode == 200) {
      return ProductsResponseModel.fromJson(data.data as Map<String, dynamic>);
  ...
  Future<ProductsResponseModel> getStoreProfileItems(
    ...
    final data = await apiService.get(
      endPoint:
          '${EndPoint.getStoreProductsUrl}${HiveStorage.get(HiveKeys.idToken)}${EndPoint.page}$page${EndPoint.pageSize}$pageSize',
    );
    print(data);
    if (data.statusCode == 200) {
      return ProductsResponseModel.fromJson(data.data as Map<String, dynamic>);
